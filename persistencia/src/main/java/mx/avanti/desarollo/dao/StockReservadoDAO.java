    ...
    public int obtenerReservado(int idArticulo, LocalDate fecha) {
        ...
        try {
            Object single = entityManager.createNativeQuery(
                    "SELECT cantidad_reservada FROM stock_reservado_diario WHERE idarticulo = :id AND fecha = :f")
                    .setParameter("id", idArticulo)
                    .setParameter("f", Date.valueOf(fecha))
                    .getSingleResult();
            if (single == null) return 0;
            if (single instanceof Number) return ((Number) single).intValue();
